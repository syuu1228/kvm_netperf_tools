...
if ARGV.size < 1
	puts "format_kvm_stat.rb [interface]"
	exit 1
end
...
avg = File.new("kvm_stat_#{IF}_avg.csv", 'w')
avg.write HEADER
min = File.new("kvm_stat_#{IF}_min.csv", 'w')
min.write HEADER
max = File.new("kvm_stat_#{IF}_max.csv", 'w')
max.write HEADER
c['cpus'].each do |cpu|
	c['vms'].each do |v|
		...
		log = File.new("kvm_stat.#{IF}#{v}-#{cpu}.log")
		cnt = 0
		log.each do |l|
